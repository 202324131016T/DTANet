...
class ResNet(nn.Module):
    def __init__(self):
        ...
        self.resconv1_x = ResNet_basic_block0(in_channels=128, out_channels=128, kernel_size=k1, kernel_size2=k2) # (N, C, L)->(N, C, L)
        self.resconv2_x = ResNet_basic_block1(in_channels=128, out_channels=128, kernel_size=k1_, kernel_size2=k2_) # (N, C, L)->(N, C, L=50) L*0.5
        self.resconv3_x = ResNet_basic_block1(in_channels=128, out_channels=128, kernel_size=k1_, kernel_size2=k2_) # (N, C, L)->(N, C, L=25) L*0.5
        # L->L 卷积最低L
        self.resconv1_x_ = ResNet_basic_block0(in_channels=128, out_channels=128, kernel_size=k1, kernel_size2=k2)
        self.convt1_x = nn.ConvTranspose1d(in_channels=128, out_channels=128, kernel_size=kt, padding=pt, stride=st) # (N, C, L)->(N, C, L=50) L*2
        self.resconv4_x = ResNet_basic_block0(in_channels=128, out_channels=128, kernel_size=k1, kernel_size2=k2) # (N, C, L)->(N, C, L)
        ...
        self.resconv5_x = ResNet_basic_block0(in_channels=128, out_channels=300, kernel_size=k1, kernel_size2=k2) # (N, C, L)->(N, C, L)

        k1, k2 = (9, 9)
        k1_, k2_ = (10, 9)
        kt, pt, st = (10, 4, 2)
        self.resconv1_y = ResNet_basic_block0(in_channels=128, out_channels=128, kernel_size=k1, kernel_size2=k2) # (N, C, L)->(N, C, L)
        self.resconv2_y = ResNet_basic_block1(in_channels=128, out_channels=128, kernel_size=k1_, kernel_size2=k2_) # (N, C, L)->(N, C, L=50) L*0.5
        self.resconv3_y = ResNet_basic_block1(in_channels=128, out_channels=128, kernel_size=k1_, kernel_size2=k2_) # (N, C, L)->(N, C, L=25) L*0.5
        # L->L 卷积最低L
        self.resconv1_y_ = ResNet_basic_block0(in_channels=128, out_channels=128, kernel_size=k1, kernel_size2=k2)  # (N, C, L)->(N, C, L)
        self.convt1_y = nn.ConvTranspose1d(in_channels=128, out_channels=128, kernel_size=kt, padding=pt, stride=st) # (N, C, L)->(N, C, L=50) L*2
        self.resconv4_y = ResNet_basic_block0(in_channels=128, out_channels=128, kernel_size=k1, kernel_size2=k2) # (N, C, L)->(N, C, L)
        ...
        self.resconv5_y = ResNet_basic_block0(in_channels=128, out_channels=300, kernel_size=k1, kernel_size2=k2) # (N, C, L)->(N, C, L)
...
class MHAT(nn.Module):
    def __init__(self, n_head=2, d_k=128, d_v=64, d_x=128, d_o=128):
        ...
        self.conv2 = nn.Conv1d(in_channels=d_o,
                               ...
                               stride=1,
                               padding=0)
        self.bn = nn.BatchNorm1d(num_features=d_o)
